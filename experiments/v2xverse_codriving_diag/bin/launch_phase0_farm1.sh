...
JOB_FILE="${JOB_FILE:-experiments/v2xverse_codriving_diag/jobs_phase0_farm1.tsv}"

STAMP="$(date +%Y%m%d_%H%M%S)"
OUT_ROOT="${OUT_ROOT:-$ROOT/experiments/v2xverse_codriving_diag/results/phase0_farm1_$STAMP}"

python experiments/v2xverse_codriving_diag/launch_local_queue.py \
  --repo-root "$ROOT" \
  --jobs "$JOB_FILE" \
  --gpus "${V2X_GPU_LIST:-1,2,3}" \
  --out-root "$OUT_ROOT" \
...
if [ "${RUN_PHASE1_AFTER:-1}" = "1" ]; then
  ...
  PHASE1_OUT_ROOT="${PHASE1_OUT_ROOT:-$ROOT/experiments/v2xverse_codriving_diag/results/phase1_pilot_farm1_$PHASE1_STAMP}"
  python experiments/v2xverse_codriving_diag/launch_local_queue.py \
    --repo-root "$ROOT" \
    --jobs "${PHASE1_JOB_FILE:-experiments/v2xverse_codriving_diag/jobs_phase1_pilot_farm1.tsv}" \
    --gpus "${V2X_GPU_LIST:-1,2,3}" \
