...
ENV_BIN="${V2XVERSE_ENV_BIN:-/data/adas/e2e/envs/v2xverse/bin}"
cd "$ROOT"

export PATH="$ENV_BIN:$PATH"
export PYTHONPATH="$ROOT:${PYTHONPATH:-}"
export WANDB_PROJECT="${WANDB_PROJECT:-v2xverse-codriving-zero-shot}"
JOB_FILE="${JOB_FILE:-experiments/v2xverse_codriving_diag/jobs_phase1_pilot.tsv}"

STAMP="$(date +%Y%m%d_%H%M%S)"
OUT_ROOT="${OUT_ROOT:-/data/adas/e2e/experiments/v2xverse_codriving_diag/results/phase1_pilot_cps_$STAMP}"
EXTRA_ARGS=()
