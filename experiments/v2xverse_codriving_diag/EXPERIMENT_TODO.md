...
Last checked: 2026-07-07 10:10 KST. FARM1 phase1 pilot root `phase1_pilot_farm1_20260706_183438` remains at launcher status `done=3,running=3`: completed jobs are `latency_det/s1/all_shifted`, `latency_det/s3/all_shifted`, and `latency_jitter/s3/all_shifted`; running jobs are `latency_jitter/stress/rsu_shifted_only` at `progress 3250/3560`, `frame_lost_hold/s2/rsu_shifted_only` at `progress 3450/3560`, and `frame_lost_hold/s2/vehicle_shifted_only` at `progress 3425/3560`. FARM9 phase1 pilot root `phase1_pilot_farm9_20260706_181437` remains at launcher status `done=6,running=1`: `compound_photo_comm/mid/all_shifted` is running at `progress 1750/3560`. CPS watcher PID `2611377` remains active; CPS phase1 root `phase1_pilot_cps_20260707_025118` advanced to status `done=3,running=1`: duplicate `latency_jitter/s3/all_shifted` completed cleanly with `summary.json`, and duplicate `latency_jitter/stress/rsu_shifted_only` launched on GPU 0, reaching `progress 175/3560`. H200 `sub_vehicle_stg2` r43 is still running at epoch 14/30 iter 738/761 with recent hard error count 0 and max checkpoint epoch 13. The only recent V2XVerse matching lines are non-fatal delayed-source replacement warnings.
