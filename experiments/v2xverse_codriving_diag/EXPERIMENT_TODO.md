...
Last checked: 2026-07-07 02:45 KST. FARM1 retry `phase0_null_all_image_seed0` is still running on FARM1 GPU 3 under `phase0_farm1_20260706_085007`; latest `run.log` reached `progress 2275/3560`, no hard error pattern observed, `summary.json` is not present yet, and the chained command remains set to aggregate phase 0 then launch FARM1 phase 1 pilot on success. FARM9 retry `phase0_null_vehicle_only_seed0` is still running on FARM9 GPU 1 under `phase0_farm9_20260706_085007`; latest `run.log` reached `progress 2650/3560`, no hard error pattern observed, `summary.json` is not present yet, and the chained command remains set to aggregate phase 0 then launch FARM9 phase 1 pilot on success. CPS watcher PID `2604297` found a free GPU at 2026-07-07 02:31 KST and launched `phase1_pilot_cps_20260707_023153`; the root now has 10 failed launcher-status rows plus one stale `running` row after the active job was stopped, and there is no active `launch_local_queue.py` or `run_planner_diag.py` process for that root. First concrete error was `AttributeError: module 'torch' has no attribute 'concat'` in the CoDriving communication path. Applied a PyTorch compatibility patch replacing actual `torch.concat` calls with `torch.cat` in `opencood/models/comm_modules/where2comm_v0.py`, `opencood/models/comm_modules/codriving.py`, `opencood/models/fuse_modules/where2comm_attn.py`, and `opencood/models/fuse_modules/codriving_attn.py`; copied the patched files to FARM and CPS mirrors, verified local/CPS/FARM `py_compile`, and verified CPS has no remaining actual `torch.concat` calls in those files. Restarted the CPS phase1 watcher as PID `2611377`; it is currently waiting because no CPS GPU is free (`mem<=2048MiB`, `util<=20%` threshold). First CPS `progress 25/...` remains pending.
